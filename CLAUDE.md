...
- **Citation verification accepts derived values.** `_candidate_values` in `citation_check.py` returns `(base, derived)`. A claim number is valid if it matches a `derived` value (whitelisted `_PAIR_OPS` arithmetic on semantically-typed evidence — `price` diff/change-%, `magnitude` ratio/margin, P/E, shares, and `macro_scalar` **diff** for rate spreads / yield-curve slope / inflation gaps), kept **sign-strict**; or it matches a `base` (first-hand) value by **magnitude** — sign-agnostic, so a claim's `+X` matches a stored `±X` (Chinese 「賣超490萬」 keeps direction in the wording, writes the number positive, while chips evidence stores `-4,904,284`). Magnitude tolerance is NOT relaxed (a 10× slip still fails). Sign mis-judgment that magnitude-matching lets through (a stored `-50M` loss cited as 「獲利 50M」) is by-design caught downstream — LLM auditor → debate → synthesis → human gate — not by cite-check, which only certifies the *number* traces to evidence. Bare integers (years, SMA windows) are not verified. Macro evidence (`category=macro`) was previously excluded from all derivation — see #84.
