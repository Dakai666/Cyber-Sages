...
- **Personas are pure data.** Drop a `personas/*.yaml` (philosophy/focus/voice/weight) to add a sage — zero code. Higher `weight` sages are seated first when `--sages N` truncates. Current roster (18): value/quality (Buffett, Munger, Graham, Damodaran, Lynch, Burry, Wood), trading (Livermore, Minervini, Raschke, Druckenmiller), tail-risk (Taleb), and the Phase 4.5 archetype additions — **Chanos** (forensic short-seller, `value`, hunts fudged numbers; 2 hard rules), **Icahn** (activist, `value`, sum-of-parts, pure SOP), **Trump** (policy-catalyst, `trading`+`value`, `weight=0.7`), **Soros** (reflexive macro, `trading`+`value`, `weight=1.05`, bets big when odds skew), **Roaring Kitty** (retail-sentiment/squeeze, `trading` only, `epoch=2021`, 2 short-interest rules), **Son** (power-law concentrated gambler, `value`, `weight=1.0`). Soros/Son are deliberately weighted to **counterbalance the conservative skew** (value-discipline + Taleb + Chanos all default to lower conviction).
- **Seating is weight-truncated *after* horizon filtering** (`council.py`: `applicable[:n_sages]`). With default `--sages 10` and 14 value-eligible sages, the lowest-weight ones (currently Taleb/Icahn/Wood/Trump, in tie-broken order) are **dropped from value runs**; trading runs seat all 8 trading-eligible. To force a specific low-weight voice (e.g. Trump) into a value council, raise `--sages`. This is the deliberate tension of a low-weight persona: light tally influence ⇄ not guaranteed a seat. Future **保守/激進 × 短期/長期 quadrant selection** (see `docs/ROADMAP.md` Phase 4.5) will let users compose the council by archetype instead of relying on weight-truncation.
